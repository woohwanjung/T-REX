...
class TransformerTokenizer():
    ...
    def __init__(self, model_class, model_name, transformer_model):
        super().__init__()
        self.model_name = model_name

        if transformer_model == "bert":
            self.tokenizer = BertTokenizer.from_pretrained(self.model_name)
        elif transformer_model == "transfoxl":
            self.tokenizer = TransfoXLTokenizer.from_pretrained(self.model_name)
        ...
        self.words = [None]*len(self.tokenizer.vocab)
        for w , wid in self.tokenizer.vocab.items():
            self.words[wid] = w


        self.max_length_actual = 0
    ...
    def convert_tokens_to_ids(self, tokens, pad=True,max_len = -1):
        if max_len <0:
    ...
    def subword_tokenize(self, tokens, max_len =-1):
        ...
        if max_len < 0:
            max_len = 512
        eff_max_len = max_len -3

        subwords = list(map(self.tokenizer.tokenize, tokens))

        subword_lengths = list(map(len, subwords))
        subwords_flatten = list(self.flatten(subwords))
        self.max_length_actual = max(self.max_length_actual, len(subwords_flatten))
        subwords = [self.CLS] + subwords_flatten[:eff_max_len] + [self.SEP]
        token_start_idxs = 1 + np.cumsum([0] + subword_lengths[:-1])
        token_start_idxs[token_start_idxs > eff_max_len] = eff_max_len
        return subwords, token_start_idxs

    def subword_tokenize_to_ids(self, tokens, max_len = -1):
        """Segment each token into subwords while keeping track of
        ...
        subword_ids, mask = self.convert_tokens_to_ids(subwords, max_len = max_len)
